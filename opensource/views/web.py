...
class ProjectDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # line_chat           = pygal.Line(x_label_rotation=90, width=600, height=300, pretty_print=True,
    ...
    def get_chart(self, stats):
        ...
        line_chat.x_labels = map(lambda x: x.datetime.strftime("%Y-%m-%d"), stats)
        star_se = df.star.diff().fillna(0)
        line_chat.add('star', [row for row in star_se])

        return line_chat.render(show_dots=True),
